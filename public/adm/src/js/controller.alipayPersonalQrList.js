(function() {
  app.controller("editAlipayPersonalQrcode", [
    "$scope",
    "http",
    function(s, http) {
      s.id;
      s.qrcode = { content: "", type: "2" };
  ...
  app.controller("alipayPersonalQrList", [
    "$scope",
    "$rootScope",
    "Table",
    "http",
    ...
    function(s, rs, Table, http, ngDialog, $stateParams, tip) {
      s.checkValue = [];
      s.table = Table.init({ link: "/admin/singleQrcodes/" + 2, query: {} });
      s.table.getList();
      ...
      s.editQR = function(qrcode) {
        ngDialog
          .open({
            template: "template/alipayPersonalQrDialog.html",
            controller: "editAlipayPersonalQrcode",
            data: { qrcode: qrcode }
          })
      ...
      s.updateStatus = function(list, status) {
        ...
        ngDialog
          ...
          .closePromise.then(function(data) {
            if (data.value && data.value == "CONFIRM") {
              list.status = status;
              http.post("/admin/updateSingleQrcode/" + list.id, list, function(
                res
              ) {
                if (res.status == 0) {
                  tip.success("操作成功!");

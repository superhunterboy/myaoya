(function() {
    app.controller("editWechatPersonalQr", [
      "$scope",
      "http",
      function(s, http) {
        s.id;
        s.qrcode = { content: "", type: "1" };
    ...
    app.controller("wechatPersonalQrList", [
      "$scope",
      "$rootScope",
      "Table",
      "http",
      ...
      function(s, rs, Table, http, ngDialog, $stateParams, tip) {
        s.checkValue = [];
        s.table = Table.init({ link: "/admin/singleQrcodes/" + 1, query: {} });
        s.table.getList();
        ...
        s.editQR = function(qrcode) {
          ngDialog
            .open({
              ...
              controller: "editWechatPersonalQr",
              data: { qrcode: qrcode }
            })

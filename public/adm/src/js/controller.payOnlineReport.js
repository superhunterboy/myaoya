(function () {
    app.controller("payOnlineReport", [
        "$scope",
        "$rootScope",
        "$filter",
        "Table",
        ...
        function (s, rs, $filter, Table, http,tip) {
            http.get("/admin/getReportItems?type=2", {}, function (res) {
                s.reportItems = res;
            });
            var time=new Date(new Date(new Date()-12*60*60*1000).toLocaleDateString());
            s.table = Table.init({link: "/admin/getPayOnlineReport",callback:function (res) {
            ...
            s.export=function () {
                ...
                window.open("/admin/getPayOnlineReport?isExport=1"+query);
            };
        }
    ]);
})()
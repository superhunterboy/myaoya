...
class BhController extends BaseController
{
    /**
     * 支付异步回调
    ...
    public function callback(Request $request, Response $response, $args)
    {
        ...
        $this->logger->addInfo('Bh payment callback data:', $returnData);

        $pay = Pay::where('order_no', '=', $returnData['orderid'])->where('money', '=', $returnData['ovalue'])->first();
        if(empty($pay->notify_url)){    //##正常
            if (isset($returnData['orderid']) && $returnData['orderid']) {
                $order_no = $returnData['orderid']; // 上传订单号
                $money = sprintf("%.2f", $returnData['ovalue']); // 金额
                $state = $returnData['restate'] ?? ''; // 支付状态
                // 加锁防止并发回调
                    ...
                    if ($pay) {
                        if ($pay->vendor_id && $pay->company_id) {
                            ...
                            if ($vendor && $company) {
                                ...
                                $returnMsg = 'failure';
                                $verify = Bh::getInstance($conf)->verifySign($returnData);
                                if ($verify) {
                                    $status = 1;
                                    if ($state == 0) {
                                        $status = 0;

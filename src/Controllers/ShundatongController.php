...
class ShundatongController extends BaseController
{
    /**
     * 支付异步回调
    ...
    public function callback(Request $request, Response $response, $args)
    {
        ...
        //日志
        $this->logger->addInfo('Shundatong payment callback data:', $returnData);
        if (isset($returnData['outTradeNo']) && $returnData['outTradeNo']) {
            $order_no = $returnData['outTradeNo'];
            $money    = $returnData['amount'];
            $opstate  = $returnData['returnCode'] ?? '';
                ...
                if ($pay) {
                    if ($pay->vendor_id && $pay->company_id) {
                        ...
                        if ($vendor && $company) {
                            ...
                            $verify              = false;
                            $returnMsg           = 'fail';
                            $verify              = Shundatong::getInstance($conf)->verifySign($returnData);
                            if ($verify) {
                                $status = 1;
                                if ($opstate == 2) {
                                    $status = 0;
                                }
                                $pay->vendor_order_no = $returnData['outTradeNo'];
                                $pay->status          = $status;
                                ...
                                if ($res) {
                                    ...
                                    if ($status == 0 && $pay->recharge_status == 0 && $company->is_autorecharge == 1 && !empty($company->autorecharge_url)) {
                                        ...
                                        if ($token) {
                                            ...
                                            $pay->save();
                                            // 记录日志
                                            $this->logger->addInfo('Shundatong payment auto recharge [resque job id=' . $token . '] data:', $requestParams);
                                        }
                                    }
                                    // Redis 解锁

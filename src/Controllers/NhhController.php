...
class NhhController extends BaseController
{
    /**
     * 支付异步回调
    ...
    public function callback(Request $request, Response $response, $args)
    {
        ...
        $this->logger->addInfo('Nhh payment callback data:', $returnData);
        $pay = Pay::where('order_no', '=', $returnData['out_trade_no'])->where('money', '=', $returnData['amount'])->first();
                    ...
                    if ($pay) {
                        if ($pay->vendor_id && $pay->company_id) {
                            ...
                            if ($vendor && $company) {
                                ...
                                $returnMsg = 'fail';
                                $verify = Cy::getInstance($conf)->verifySign($returnData);
                                ...
                                if ($verify) {
                                    ...
                                    $pay->vendor_order_no = $returnData['out_channel_no'];
                                    $pay->status = $status;
                                    ...
                                    if ($res) {
                                        ...
                                        if ($status == 0 && $pay->recharge_status == 0 && $company->is_autorecharge == 1 && !empty($company->autorecharge_url)) {
                                            ...
                                            if ($token) {
                                                ...
                                                $pay->save();
                                                // 记录日志
                                                $this->logger->addInfo('Bh payment auto recharge [resque job id=' . $token . '] data:', $requestParams);
                                            }
                                        }
                                        // Redis 解锁

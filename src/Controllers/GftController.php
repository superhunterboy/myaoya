...
class GftController extends BaseController
{
    /**
     * 支付异步回调
    ...
    public function callback(Request $request, Response $response, $args)
    {
        ...
        // }
        //日志
        $this->logger->addInfo('Gft payment callback data:', $returnData);
        ...
        if (isset($returnData['mchOrderNo']) && $returnData['mchOrderNo']) {
            ...
            {
                $money = sprintf("%.2f", intval($returnData['amount'] / 100)); // 金额
            }
                ...
                if ($pay) {
                    if ($pay->vendor_id && $pay->company_id) {
                        ...
                        if ($vendor && $company) {
                            ...
                            $returnMsg           = 'failure';
                            $verify              = Gft::getInstance($conf)->verifySign($returnData);
                            ...
                            if ($verify) {
                                ...
                                $pay->vendor_order_no = $returnData['platformOrderId'];
                                $pay->status          = $status;
                                ...
                                if ($res) {
                                    ...
                                    if ($status == 0 && $pay->recharge_status == 0 && $company->is_autorecharge == 1 && !empty($company->autorecharge_url)) {
                                        ...
                                        if ($token) {
                                            ...
                                            $pay->save();
                                            // 记录日志
                                            $this->logger->addInfo('Gft payment auto recharge [resque job id=' . $token . '] data:', $requestParams);
                                        }
                                    }
                                    // Redis 解锁

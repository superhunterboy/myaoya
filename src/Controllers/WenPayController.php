...
class WenPayController extends BaseController
{
    /**
     * 支付异步回调
    ...
    public function callback(Request $request, Response $response, $args)
    {
        ...
        //     "fxtime": "1557473875",
        //     "fxstatus": "1",
        //     "fxsign": "17ba6db7b911ef133d51b75b0ddb2766"
        // }
        $this->logger->addInfo('WenPay payment callback data:', $returnData);
        $pay = Pay::where('order_no', '=', $returnData['fxddh'])->where('money', '=', intval($returnData['fxfee']))->first();

        if (empty($pay->notify_url)) {
            //##正常
            if (isset($returnData['fxddh']) && $returnData['fxddh']) {
                $order_no = $returnData['fxddh']; // 上传订单号
                $money    = intval($returnData['fxfee']); // 金额
                $state    = $returnData['fxstatus'] ?? ''; // 支付状态
                // 加锁防止并发回调
                    ...
                    if ($pay) {
                        if ($pay->vendor_id && $pay->company_id) {
                            ...
                            if ($vendor && $company) {
                                ...
                                $returnMsg           = 'fail';
                                $verify              = WenPay::getInstance($conf)->verifySign($returnData);
                                if ($verify) {
                                    $status = 1;
                                    if ($state == 1) {
                                        $status = 0;
                                    }
                                    $pay->vendor_order_no = $returnData['fxddh'];
                                    $pay->status          = $status;
                                    ...
                                    if ($res) {
                                        ...
                                        if ($status == 0 && $pay->recharge_status == 0 && $company->is_autorecharge == 1 && !empty($company->autorecharge_url)) {
                                            ...
                                            if ($token) {
                                                ...
                                                $pay->save();
                                                // 记录日志
                                                $this->logger->addInfo('WenPay payment auto recharge [resque job id=' . $token . '] data:', $requestParams);
                                            }
                                        }
                                        // Redis 解锁
    ...
    public function notify(Request $request, Response $response, $args)
    {
        ...
        $this->logger->addInfo('WenPay payment notify data:', $returnData);

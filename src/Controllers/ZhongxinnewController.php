...
class ZhongxinnewController extends BaseController
{
    /**
     * 支付异步回调
    ...
    public function callback(Request $request, Response $response, $args)
    {
        ...
        $this->logger->addInfo('Zhongxinnew payment callback data:', $returnData);
        $pay = Pay::where('order_no', '=', $returnData['order'])->where('money', '=', $returnData['amount'])->first();
        if(empty($pay->notify_url)){    //##正常
            if (isset($returnData['order']) && $returnData['order']) {
                $order_no = $returnData['order']; // 上传订单号
                    ...
                    if ($pay) {
                        if ($pay->vendor_id && $pay->company_id) {
                            ...
                            if ($vendor && $company) {
                                ...
                                $returnMsg = 'fail';
                                $verify = Zhongxinnew::getInstance($conf)->verifySign($returnData);
                                ...
                                if ($verify) {
                                    ...
                                    //if ($state == 00000) {
                                        $status = 0;
                                    //}
                                    $pay->vendor_order_no = $returnData['transaction_id'];
                                    $pay->status = $status;
                                    ...
                                    if ($res) {
                                        ...
                                        if ($status == 0 && $pay->recharge_status == 0 && $company->is_autorecharge == 1 && !empty($company->autorecharge_url)) {
                                            ...
                                            if ($token) {
                                                ...
                                                $pay->save();
                                                // 记录日志
                                                $this->logger->addInfo('Zhongxinnew payment auto recharge [resque job id=' . $token . '] data:', $requestParams);
                                            }
                                        }
                                        // Redis 解锁

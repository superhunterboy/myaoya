...
class KailiantongController extends BaseController
{
    ...
    public function callback(Request $request, Response $response, $args)
    {
        ...
        {
            "issuerId":"",
            "orderNo":"201712191020402322040",
            "payDatetime":"20171219111837",
            "ext1":"",
            "mchtOrderId":"201712191118015825",
            "payResult":"1",
            "orderAmount":"1",
            "ext2":"",
            "signMsg":"68591ED7097DA2A4C2BE3C7C3A04DDB0",
            "signType":"0",
            "payType":"20",
            "merchantId":"100020091219001",
            "language":"1",
            "orderDatetime":"20171219102040",
            "version":"v1.0"
        }
        */

        //日志
        $this->logger->addInfo('Kailiantong payment callback data:', $returnData);

        $returnMsg = '';
        if (isset($returnData['orderNo']) && $returnData['orderNo']) {

            $order_no = $returnData['orderNo']; //上传订单号
            $money    = $returnData['orderAmount'] ?? 0; //金额
            $money    = sprintf("%.2f", $money/100); //格式化
            $state    = $returnData['payResult'] ?? ''; //支付状态
            $orderMer = $returnData['mchtOrderId'] ?? ''; //流水号
                ...
                if ($pay) {
                    if ($pay->vendor_id && $pay->company_id) {
                        ...
                        if ($vendor && $company) {
                            ...
                            if ($pay_type == 31) {
                                $verify = Kailiantong::getInstance($conf)->verifySign($returnData);
                                if ($verify) {
                                    $status = 0;
                                    ...
                                    if ($res) {
                                        ...
                                        if ($status == 0 && $pay->recharge_status == 0 && $company->is_autorecharge == 1 && !empty($company->autorecharge_url)) {
                                            ...
                                            if ($token) {
                                                ...
                                                $pay->save();
                                                // 记录日志
                                                $this->logger->addInfo('Kailiantong payment auto recharge [resque job id=' . $token . '] data:', $requestParams);
                                            }
                                        }
    ...
    public function notify(Request $request, Response $response, $args)
    {
        ...
        // 日志
        $this->logger->addInfo('Kailiantong payment notify data:', $returnData);

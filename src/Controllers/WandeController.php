...
class WandeController extends BaseController
{
    /**
     * 支付异步回调
    ...
    public function callback(Request $request, Response $response, $args)
    {
        ...
        $this->logger->addInfo('wande payment callback data:', $returnData);
        $pay = Pay::where('order_no', '=', $returnData['mchOrderNo'])->where('money', '=', $returnData['amount'] / 100)->first();

        if (empty($pay->notify_url)) {
            //##正常
            if (isset($returnData['mchOrderNo']) && $returnData['mchOrderNo']) {
                $order_no = $returnData['mchOrderNo']; // 上传订单号
                $money    = intval($returnData['amount'] / 100); // 金额
                    ...
                    if ($pay) {
                        if ($pay->vendor_id && $pay->company_id) {
                            ...
                            if ($vendor && $company) {
                                ...
                                $returnMsg           = 'fail';
                                $verify              = Wande::getInstance($conf)->verifySign($returnData);
                                if ($verify) {
                                    $status = 1;
                                    if ($state == 2) {
                                        $status = 0;
                                    }
                                    $pay->vendor_order_no = $returnData['mchOrderNo'];
                                    $pay->status          = $status;
                                    ...
                                    if ($res) {
                                        ...
                                        if ($status == 0 && $pay->recharge_status == 0 && $company->is_autorecharge == 1 && !empty($company->autorecharge_url)) {
                                            ...
                                            if ($token) {
                                                ...
                                                $pay->save();
                                                // 记录日志
                                                $this->logger->addInfo('Wande payment auto recharge [resque job id=' . $token . '] data:', $requestParams);
                                            }
                                        }
                                        // Redis 解锁
    ...
    public function notify(Request $request, Response $response, $args)
    {
        ...
        $this->logger->addInfo('Wande payment notify data:', $returnData);

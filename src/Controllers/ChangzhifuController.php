...
class ChangzhifuController extends BaseController
{
    /**
     * 支付异步回调
    ...
    public function callback(Request $request, Response $response, $args)
    {
        ...
        //日志
        $this->logger->addInfo('Changzhifu payment callback data:', $returnData);

        if (isset($returnData['orderid']) && $returnData['orderid']) {

            $notifyType = $returnData['type'];
            $order_no   = $returnData['orderid'];
            $money      = sprintf("%.2f", $returnData['ovalue']);
                ...
                if ($pay) {
                    if ($pay->vendor_id && $pay->company_id) {
                        ...
                        if ($vendor && $company) {
                            ...
                            $verify              = false;
                            $returnMsg           = 'failure';
                            $verify              = Changzhifu::getInstance($conf)->verifySign($returnData);
                            if ($verify) {
                                $status = 1;
                                if ($opstate == '0') {
                                    $status = 0;
                                ...
                                if ($res) {
                                    ...
                                    if ($status == 0 && $pay->recharge_status == 0 && $company->is_autorecharge == 1 && !empty($company->autorecharge_url)) {
                                        ...
                                        if ($token) {
                                            ...
                                            $pay->save();
                                            // 记录日志
                                            $this->logger->addInfo('Changzhifu payment auto recharge [resque job id=' . $token . '] data:', $requestParams);
                                        }
                                    }
                                    // Redis 解锁
    ...
    public function notify(Request $request, Response $response, $args)
    {
        ...
        $this->logger->addInfo('Changzhifu payment notify data:', $returnData);

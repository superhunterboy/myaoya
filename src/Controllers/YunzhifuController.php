...
class YunzhifuController extends BaseController
{
    ...
    public function callback(Request $request, Response $response, $args)
    {
        ...
        */
        ...
        //日志
        $this->logger->addInfo('Yunzhifu payment callback data:', $returnData);
        ...
        if (isset($returnData['orderid']) && $returnData['orderid']) {
            ...
            $orderMer = $returnData['p_id'] ?? date('YmdHis'); //流水号
                ...
                if ($pay) {
                    if ($pay->vendor_id && $pay->company_id) {
                        ...
                        if ($vendor && $company) {
                            ...
                            if ($pay_type == 37) {
                                $verify = Yunzhifu::getInstance($conf)->verifySign($returnData);
                                if ($verify) {
                                    $status = 0;
                                    ...
                                    if ($res) {
                                        ...
                                        if ($status == 0 && $pay->recharge_status == 0 && $company->is_autorecharge == 1 && !empty($company->autorecharge_url)) {
                                            ...
                                            if ($token) {
                                                ...
                                                $pay->save();
                                                // 记录日志
                                                $this->logger->addInfo('Yunzhifu payment auto recharge [resque job id=' . $token . '] data:', $requestParams);
                                            }
                                        }
    ...
    public function notify(Request $request, Response $response, $args)
    {
        ...
        // 日志
        $this->logger->addInfo('Yunzhifu payment notify data:', $returnData);

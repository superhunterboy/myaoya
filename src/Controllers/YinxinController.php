...
class YinxinController extends BaseController
{
    /**
     * 支付异步回调
    ...
    public function callback(Request $request, Response $response, $args)
    {
        ...
        //日志
        $this->logger->addInfo('Yinxin payment callback data:', $returnData);

        if (isset($returnData['out_trade_no']) && $returnData['out_trade_no']) {

            $order_no   = $returnData['out_trade_no'];
            $money      = sprintf("%.2f", $returnData['money']);
            $opstate    = $returnData['trade_status'] ?? '';
                ...
                if ($pay) {
                    if ($pay->vendor_id && $pay->company_id) {
                        ...
                        if ($vendor && $company) {
                            ...
                            $verify              = false;
                            $returnMsg           = 'fail';
                            $verify              = Yinxin::getInstance($conf)->verifySign($returnData);
                            if ($verify) {
                                $status = 1;
                                ...
                                if ($res) {
                                    ...
                                    if ($status == 0 && $pay->recharge_status == 0 && $company->is_autorecharge == 1 && !empty($company->autorecharge_url)) {
                                        ...
                                        if ($token) {
                                            ...
                                            $pay->save();
                                            // 记录日志
                                            $this->logger->addInfo('Yinxin payment auto recharge [resque job id=' . $token . '] data:', $requestParams);
                                        }
                                    }
                                    // Redis 解锁
    ...
    public function notify(Request $request, Response $response, $args)
    {
        ...
        $this->logger->addInfo('Yinxin payment notify data:', $returnData);

...
class DaniuController extends BaseController
{
    /** -----
     * 支付异步回调
    ...
    {
        $returnData['memberid']       = $_REQUEST["memberid"]; //交易流水号
        $returnData['orderid']        = $_REQUEST["orderid"]; //交易流水号
        $returnData['transaction_id'] = $_REQUEST["transaction_id"]; //交易流水号
        $returnData['amount']         = $_REQUEST["amount"]; //交易流水号
        $returnData['datetime']       = $_REQUEST["datetime"]; //交易流水号
        $returnData['returncode']     = $_REQUEST["returncode"]; //交易流水号
        $returnData['sign']           = $_REQUEST["sign"]; //交易流水号
        $returnData['attach']         = $_REQUEST["attach"]; //交易流水号
        ...
        $this->logger->addInfo('Daniu payment callback data:', $_REQUEST);
        ...
        if (empty($pay->notify_url)) {
            ...
            if (isset($returnData['orderid']) && $returnData['orderid']) {
                ...
                $money    = sprintf("%.2f", $returnData['amount']); // 金额
                    ...
                    if ($pay) {
                        if ($pay->vendor_id && $pay->company_id) {
                            ...
                            if ($vendor && $company) {
                                ...
                                $verify              = false;
                                $returnMsg           = 'failure';
                                $verify              = Daniu::getInstance($conf)->verifySign($returnData);
                                    ...
                                    if ($res) {
                                        ...
                                        if ($status == 0 && $pay->recharge_status == 0 && $company->is_autorecharge == 1 && !empty($company->autorecharge_url)) {
                                            ...
                                            if ($token) {
                                                ...
                                                $pay->save();
                                                // 记录日志
                                                $this->logger->addInfo('Daniu payment auto recharge [resque job id=' . $token . '] data:', $requestParams);
                                            }
                                        }
                                        // Redis 解锁
                        ...
                        if ($pay->vendor_id && $pay->company_id) {
                            ...
                            if ($vendor && $company) {
                                ...
                                if ($verify) {
                                    ...
                                    $pay->remark          = json_encode($returnData);
                                    $res                  = $pay->save();
                                    $returnMsg['message'] = 'ok';
                                    $returnMsg['status']  = '200';

                                }
                                $response->getBody()->write(json_encode($returnMsg));
                            }
                        }
                    }
                }

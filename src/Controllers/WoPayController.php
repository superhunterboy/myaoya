...
class WoPayController extends BaseController
{
    /**
     * 支付异步回调
    ...
    public function callback(Request $request, Response $response, $args)
    {
        ...
        $this->logger->addInfo('WoPay payment callback data:', $returnData);
        $pay = Pay::where('order_no', '=', $returnData['externalId'])->first();

        if (empty($pay->notify_url)) {
            //##正常
            if (isset($returnData['externalId']) && $returnData['externalId']) {
                $order_no = $returnData['externalId']; // 上传订单号
                $money    = ($returnData['amount'] / 100); // 金额
                    ...
                    if ($pay) {
                        if ($pay->vendor_id && $pay->company_id) {
                            ...
                            if ($vendor && $company) {
                                ...
                                $verify              = false;
                                $returnMsg           = 'failure';
                                $verify              = WoPay::getInstance($conf)->verifySign($returnData);
                                if ($verify) {
                                    $status = 1;
                                    if ($state == 00) {
                                        $status = 0;
                                    }
                                    $pay->vendor_order_no = $returnData['seqno'];
                                    $pay->status          = $status;
                                    ...
                                    if ($res) {
                                        ...
                                        if ($status == 0 && $pay->recharge_status == 0 && $company->is_autorecharge == 1 && !empty($company->autorecharge_url)) {
                                            ...
                                            if ($token) {
                                                ...
                                                $pay->save();
                                                // 记录日志
                                                $this->logger->addInfo('WoPay payment auto recharge [resque job id=' . $token . '] data:', $requestParams);
                                            }
                                        }
                                        // Redis 解锁
    ...
    public function notify(Request $request, Response $response, $args)
    {
        ...
        $this->logger->addInfo('WoPay payment notify data:', $returnData);

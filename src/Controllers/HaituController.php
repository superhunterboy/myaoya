...
class HaituController extends BaseController
{
    /**
     * 支付异步回调
    ...
    public function callback(Request $request, Response $response, $args)
    {
        ...
        $this->logger->addInfo('haitu payment callback data:', $returnData);
        $pay = Pay::where('order_no', '=', $returnData['out_trade_no'])->where('money', '=', $returnData['total_fee']/100)->first();

        if (empty($pay->notify_url)) {
            //##正常
            if (isset($returnData['out_trade_no']) && $returnData['out_trade_no']) {
                $order_no = $returnData['out_trade_no']; // 上传订单号
                $money    = intval($returnData['total_fee']/100); // 金额
                    ...
                    if ($pay) {
                        if ($pay->vendor_id && $pay->company_id) {
                            ...
                            if ($vendor && $company) {
                                ...
                                $returnMsg           = 'fail';
                                $verify              = Haitu::getInstance($conf)->verifySign($returnData);
                                if ($verify) {
                                    $status = 0;
                                    ...
                                    if ($res) {
                                        ...
                                        if ($status == 0 && $pay->recharge_status == 0 && $company->is_autorecharge == 1 && !empty($company->autorecharge_url)) {
                                            ...
                                            if ($token) {
                                                ...
                                                $pay->save();
                                                // 记录日志
                                                $this->logger->addInfo('Haitu payment auto recharge [resque job id=' . $token . '] data:', $requestParams);
                                            }
                                        }
                                        // Redis 解锁

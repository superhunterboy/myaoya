...
class GaotongController extends BaseController
{
    // 异步回调
    public function callback(Request $request, Response $response, $args)
    {
        ...
        // 日志
        $this->logger->addInfo('Gaotong payment callback data:', $returnData);
                    ...
                    if ($pay->vendor_id && $pay->company_id) {
                        ...
                        if ($vendor && $company) {
                            ...
                            if ($pay_type == 15) {
                                ...
                                $verify = Gaotong::getInstance($conf)->verifySign($returnData);
                                    ...
                                    if ($pay_type == 15) {
                                        ...
                                        if ($status == 0 && $pay->recharge_status == 0 && $company->is_autorecharge == 1 && !empty($company->autorecharge_url)) {
                                            ...
                                            if ($token) {
                                                ...
                                                $pay->save();
                                                // 记录日志
                                                $this->logger->addInfo('Gaotong payment auto recharge [resque job id=' . $token . '] data:', $requestParams);
                                            }
                                        }
    ...
    public function notify(Request $request, Response $response, $args)
    {
        ...
        $this->logger->addInfo('Gaotong payment notify data:', $getDatas);

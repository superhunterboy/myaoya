...
class Utils
{
    ...
    /**
     * 如用户输入100元 生成金额范围是 99.00,99.01,99.02...99.99,100.00
     * @param  int $money
     * @param  int $deduction
     * @return int $randMoney  金额（单位：分）
     */
    public static function randMoney($money, $deduction = 1)
    {
        $decimal   = mt_rand(0, $deduction * 100);
        $decimal   = $decimal / 100;
        $randMoney = $money - $deduction + $decimal;
        $randMoney = $randMoney * 100; //金额（单位：分）
        return $randMoney;
    }
     ...
     * @return bool|string
     */
    public static function publicEncrypt($json, $public_key)
    {
        $encrypted = '';
    ...
    public static function md5WithRSA($data, $pkeyid)
    {
        $str       = chunk_split($pkeyid, 64, "\n");
     ...
     * @return bool|string
     */
    public static function RHPay_encryptByRSA($plaintext, $priKey)
    {
        $split_data    = str_split($plaintext, 117);
        $encrypt_array = [];
        foreach ($split_data as $part) {
            $result = openssl_private_encrypt($part, $encrypt_string, self::formatRsaKey($priKey, 'priKey'));
            if (!$result) {
                return false;
            }
            $encrypt_array[] = $encrypt_string;
     ...
     * @return bool|string
     */
    public static function RHPay_decryptByRSA($ciphertext, $priKey)
    {
        $ciphertext     = self::base64_url_safe_decode($ciphertext); //RHPay那边用Java传过来的加密内容包含特殊字符
        $split_array    = str_split(base64_decode($ciphertext), 128);
        $decrypt_string = '';
        foreach ($split_array as $part) {
            $result = openssl_private_decrypt($part, $decrypt_data, self::formatRsaKey($priKey, 'priKey'));
     ...
     * Date: NOW_TIME
     * @param $string
     * @return mixed|stringcal
     */
    public static function base64_url_safe_decode($string)
    {
     ...
     * 加密方法
     */
    public static function encryptAES_CBC($data, $key)
    {
        $iv         = $key;
        $str_padded = $data;
        ...
        return base64_encode(openssl_encrypt($str_padded, 'AES-128-CBC', $key, OPENSSL_NO_PADDING, $iv));
    }

 ...
 * 自动出款自动审核处理 Job
 */
class AutoPayOutCheckJob extends BaseJob
{
    /**
     * 当前出款平台
     * @var null
    ...
    private function doTianfubao()
    {
        ...
        if (!empty($tfb_serialno) && $serialno_state == 1) {
    ...
    private function doXifu()
    {
        ...
        if (isset($result['batchNo']) && isset($result['batchContent']) && $resultCode == 'S0001') {
            ...
            if (strpos($returnCode[12], '成功') !== false) {
                // 交易成功，修改 BBIN 后台出款单状态为 1 确定
                $crawlerRes = Crawler::getInstance()->updatePayOutData($this->crawlerUrl, [
                    'act'     => 'updateStatus',
                    'id'      => $this->payOut->wid,
    ...
    private function doAifu()
    {
        ...
        if ($result_code == '000000' && $orderNo && $resultFlag == 'S') {
            // 交易成功，修改 BBIN 后台出款单状态为 1 确定
            $crawlerRes = Crawler::getInstance()->updatePayOutData($this->crawlerUrl, [
                'act'     => 'updateStatus',
                'id'      => $this->payOut->wid,
    ...
    private function doDuobao()
    {
        ...
        if (!empty($orderId) && $status == 'success' && strpos($state, '已支付') !== false) {
            // 交易成功，修改 BBIN 后台出款单状态为 1 确定
            $crawlerRes = Crawler::getInstance()->updatePayOutData($this->crawlerUrl, [
                'act'     => 'updateStatus',
                'id'      => $this->payOut->wid,
    ...
    private function doBingo()
    {
        ...
        if ($respCode == '00' && !empty($orderId) && $state == 3) {
            // 交易成功，修改 BBIN 后台出款单状态为 1 确定
            $crawlerRes = Crawler::getInstance()->updatePayOutData($this->crawlerUrl, [
                'act'     => 'updateStatus',
                'id'      => $this->payOut->wid,
    ...
    private function doGaiya()
    {
        ...
        $status  = $payOutStatus['success'] ?? 'fail';
        $code    = $payOutStatus['code'] ?? 'fail';
        $msg     = $payOutStatus['respMsg'] ?? '';
        $state   = $payOutStatus['transStatus'] ?? '';
        $orderId = $payOutStatus['merOrderId'] ?? '';

        if (!empty($orderId) && $status == 1 && $code !== 1001) {
            // 交易成功，修改 BBIN 后台出款单状态为 1 确定
            $crawlerRes = Crawler::getInstance()->updatePayOutData($this->crawlerUrl, [
                'act'     => 'updateStatus',
                'id'      => $this->payOut->wid,
